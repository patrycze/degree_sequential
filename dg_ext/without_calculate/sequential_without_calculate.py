...
def sequential(nr, network, pp, step, graph, infectedNodes, coordinatedExecution, seeds, time, limit, timeArray):
    ...
    while(isInfecting):
                        ...
                        for k in notinfected:
                            ...
                            if (numberofneighbors >= 1):
                                ...
                                if x <= pp:
                                    ...
                                    infectedNodes.add(graph.vs[k]['name'])
        ...
        myFile = open(str(pp) + '_' + str(limit) + '_results_without_calculate.csv', 'a')
        with myFile:
            ...
            writer.writerow({'nr': nr, 'nazwa': network, 'pp': pp, 'numberOfSeeds': len(seeds), 'seeds': seeds,  'totalNumberOfSeeds': calculateNumberOfSeeds(graph), 'numberOfNodes': nodes, 'step': step,
                             'infectedPerStep': infectionsPerStep, 'infectedTotal': len(totalInfected),  'infectedTotalPercentage': len(totalInfected) / nodes * 100, 'computionalTime': [], 'limitPercentage': limit})
